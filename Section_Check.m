function varargout = Section_Check(varargin)
% SECTION_CHECK MATLAB code for Section_Check.fig
%      SECTION_CHECK, by itself, creates a new SECTION_CHECK or raises the existing
%      singleton*.
%
%      H = SECTION_CHECK returns the handle to a new SECTION_CHECK or the handle to
%      the existing singleton*.
%
%      SECTION_CHECK('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SECTION_CHECK.M with the given input arguments.
%
%      SECTION_CHECK('Property','Value',...) creates a new SECTION_CHECK or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Section_Check_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Section_Check_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Section_Check

% Last Modified by GUIDE v2.5 02-Jun-2014 16:25:45

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Section_Check_OpeningFcn, ...
                   'gui_OutputFcn',  @Section_Check_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Section_Check is made visible.
function Section_Check_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Section_Check (see VARARGIN)

% Choose default command line output for Section_Check
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Section_Check wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Section_Check_OutputFcn(hObject, eventdata, handles) 
...
bbb={'NOT OK'};
 while(1)  
       ...
       fprintf(file,'Inputs:\n\n');
       fprintf(file,'Factored shear is                             %.2f kN/m\n',factored_shear);
